...
namespace BTL.Forms.Main.Khuyenmais
{
    public partial class AddKM : Form
    {
        ...
        private void ibtnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtMaKM.Text.Trim() == "") throw new Exception("Mã khuyến mãi không được bỏ trống!");
                KhuyenMai km1 = new KhuyenMai();
                km1 = db.KhuyenMais.Find(txtMaKM.Text);
                if (km1 != null) throw new Exception("Đã có mã khuyến mại này! \n Vui lòng nhập lại! ");
                
                if (txtGiamGia.Text.Trim() == "") throw new Exception("Mã khuyến mãi không được bỏ trống!");
