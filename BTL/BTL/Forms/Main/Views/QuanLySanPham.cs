    ...
    public partial class QuanLySanPham : Form
    {
        ...
        private void hienThiData()
        {
            ...
            var ds = from s in db.SanPhams
                     select new
                     {
                         s.MaSp,s.TenSp,s.XuatXu,s.ThuongHieu,s.DonViTinh,s.Slton,s.DonGia,s.MaDmNavigation.TenDm
                     };
            foreach (var item in ds)
            {
                dgvSP.Rows.Add(item.MaSp, item.TenSp, item.XuatXu, item.ThuongHieu, item.DonViTinh, item.Slton, item.DonGia.ToString("#,###", cul.NumberFormat), item.TenDm);
            }
