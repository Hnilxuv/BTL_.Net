...
namespace BTL.Forms.Main.Views
{
    public partial class QuanLyNhapHang : Form
    {
        ...
        private void dataViewNV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtMaPhieuNhap.Text = pn[0].mapn;
            txtmacuahang.Text = pn[0].mach;
            txtmancc.Text = pn[0].mancc;
            txtngaynhap.Text = pn[0].ngaynhap.ToString("dd-MM-yyyy HH:mm:ss");
            txtthanhtoan.Text = pn[0].tt;
            txtnguoinhap.Text = pn[0].nn;
            txtmaphieudat.Text = pn[0].mapd;
            dataViewSpCanDoi.Rows.Clear();
            long tongtien = 0;
            foreach (var item in ctpn)
            {
                tongtien += (long)(item.sl * item.gia);
                dataViewSpCanDoi.Rows.Add(item.msp, item.tsp, item.sl, item.gia, item.sl * item.gia);
            }
